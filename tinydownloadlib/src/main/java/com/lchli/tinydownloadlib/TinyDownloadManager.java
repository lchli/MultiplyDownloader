...
public class TinyDownloadManager {
     ...
     * @param task
     */
    synchronized void addTask(final TinyDownloadTask task) {
    ...
    synchronized void continueTask(TinyDownloadTask task) {
        TinyDownloader downloader = runningDownloaders.get(task.uid);
        if (downloader != null) {
            LogUtils.e("task already downloading task=:" + task);
            return;
    ...
    synchronized void pauseTask(TinyDownloadTask task) {
        TinyDownloader downloader = runningDownloaders.get(task.uid);
        if (downloader == null) {
            return;
        }
    ...
    synchronized void deleteTask(final TinyDownloadTask task) {
        TinyDownloader downloader = runningDownloaders.get(task.uid);
        if (downloader != null) {
            downloader.cancel();
        }
